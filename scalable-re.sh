...
installscalable-re(){
	...
	#Finish Install
	update-grub
	echo -e "\033[42;37m[注意]\033[0m 重启VPS后，请重新运行脚本开启scalable-re \033[42;37m bash scalable-re.sh start \033[0m"
	stty erase '^H' && read -p "需要重启VPS后，才能开启scalable-re，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
		if [[ $yn == [Yy] ]]; then
